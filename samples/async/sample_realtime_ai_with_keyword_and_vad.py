...
class MyAudioCaptureEventHandler(AudioCaptureEventHandler):
    def __init__(
        ...
        self._silence_timeout = (
            10  # Silence timeout in seconds for rearming keyword detection
        )
    ...
    def send_audio_data(self, audio_data: bytes):
        ...
        if self._state == ConversationState.CONVERSATION_ACTIVE:
            ...
            asyncio.run_coroutine_threadsafe(
                self._client.send_audio(audio_data), self._event_loop
            )
    ...
    def on_speech_start(self):
        ...
        if (
            self._state == ConversationState.KEYWORD_DETECTED
            or self._state == ConversationState.CONVERSATION_ACTIVE
        ):
            asyncio.run_coroutine_threadsafe(
                self._set_state(ConversationState.CONVERSATION_ACTIVE), self._event_loop
            )
            asyncio.run_coroutine_threadsafe(
                self._cancel_silence_timer(), self._event_loop
            )

        if (
            self._client.options.turn_detection is None
            and self._event_handler.is_audio_playing()
            and self._state == ConversationState.CONVERSATION_ACTIVE
        ):
            logger.info(
                "User started speaking while assistant is responding; interrupting the assistant's response."
            )
            asyncio.run_coroutine_threadsafe(
                self._client.clear_input_audio_buffer(), self._event_loop
            )
            asyncio.run_coroutine_threadsafe(
                self._client.cancel_response(), self._event_loop
            )
    ...
    def on_speech_end(self):
        ...
        if (
            ...
            and self._client.options.turn_detection is None
        ):
            logger.debug(
                "Using local VAD; requesting the client to generate a response after speech ends."
            )
            asyncio.run_coroutine_threadsafe(
                self._client.generate_response(), self._event_loop
            )
            logger.debug("Conversation is active. Starting silence timer.")
            asyncio.run_coroutine_threadsafe(
                self._start_silence_timer(), self._event_loop
            )
    ...
    def on_keyword_detected(self, result):
        ...
        asyncio.run_coroutine_threadsafe(
            self._client.send_text("Hello"), self._event_loop
        )
        asyncio.run_coroutine_threadsafe(
            self._set_state(ConversationState.KEYWORD_DETECTED), self._event_loop
        )
    ...
    async def _start_silence_timer(self):
        ...
        self._silence_timer_task = asyncio.create_task(
            self._silence_timer_coroutine(), name="SilenceTimer"
        )
    ...
    async def _reset_state_due_to_silence(self):
        if (
            ...
            or self._event_handler.is_function_processing()
        ):
            logger.info(
                "Assistant is responding or processing a function. Waiting to reset keyword detection."
            )
        ...
        asyncio.run_coroutine_threadsafe(
            self._set_state(ConversationState.IDLE), self._event_loop
        )
...
class MyRealtimeEventHandler(RealtimeAIEventHandler):
    ...
    async def on_response_function_call_arguments_done(
        ...
        try:
            ...
            function_output = await asyncio.threads.to_thread(
                self._functions.execute, function_name, arguments_str
            )
            logger.info(f"Function output for call ID {call_id}: {function_output}")
            await self._client.generate_response_from_function_call(
                call_id, function_output
            )
