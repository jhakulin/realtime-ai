...
class MyRealtimeEventHandler(RealtimeAIEventHandler):
    ...
    def _handle_response_done(self, event: ResponseDone) -> bool:
        ...
        is_function_call_present = self._check_function_call(
        ...
        if not is_function_call_present:
            if self._is_transcription_for_audio_created is False:
                messages = self._extract_content_messages(
                    event.response.get("output", [])
                )
    ...
    def _handle_failed_response(self, response):
        ...
        logger.debug(
            f"Failed response: Type: {error_type}, Code: {error_code}, Message: {error_message}"
        )
    ...
    try:
        ...
        while not stop_event.is_set():
            try:
                ...
                if (
                    user_input == TEXT_MODALITY_COMMAND
                    or user_input == AUDIO_AND_TEXT_MODALITY_COMMAND
                ):
                    # Update the modality based on the user input
                    new_modality = (
                        "text" if user_input == TEXT_MODALITY_COMMAND else "audio_text"
                    )
                    options.modalities = (
                        ["text"] if new_modality == "text" else ["audio", "text"]
                    )
...
def print_instructions():
    ...
    print(
        " - Type '/audio_text' to switch to audio_text modality. In this sample, only audio output is supported."
    )
    print(" - Type your message and press Enter to send it to the assistant.")
    print(" - Type '/text' to switch to text modality.")
